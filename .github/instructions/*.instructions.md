...
### General Python
- No `except: pass` or bare `except Exception` without logging. Every caught exception must at minimum `print(f"[module] error: {e}")` — consistent with existing patterns.
